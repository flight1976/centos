...
find /etc/yum.repos.d -type f -name CentOS-PowerTools.repo -exec sed -i 's/#baseurl=http:\/\/mirror.centos.org/baseurl=http:\/\/mirror01.idc.hinet.net/' {} \;
find /etc/yum.repos.d -type f -name CentOS-PowerTools.repo -exec sed -i 's/^mirrorlist/#mirrorlist/' {} \;


find /etc/yum.repos.d -type f -name epel.repo -exec sed -i 's/#baseurl=https:\/\/download.fedoraproject.org\/pub/baseurl=http:\/\/mirror01.idc.hinet.net/' {} \;
find /etc/yum.repos.d -type f -name epel.repo -exec sed -i 's/^metalink/#mtalink/' {} \;

dnf config-manager --set-enabled PowerTools

# Install some daily use packages.
dnf -y install net-tools wget curl telnet lftp tcpdump vim iptables-services screen
